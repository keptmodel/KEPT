...
class CodeKnowledgeGraph:
    ...
    def add_relation_encode(self, pl_txt: str, max_length=256,pl_location: str="",id=0):
        ...
        remain_tokens_len = max_length - 2 - len(tokens) if len(tokens) < max_length - 2 else 0
            ...
            if token == self.entity_flag:
                if last_tag_location is None:
                    ...
                    entity_loc_list.append(
                        {'entity': entity_list[entity_index], 'location': (last_tag_location+1, abs_pos - 1 + 1)})
                    last_tag_location = None
                    entity_index += 1
            else:
                tokens.append(token)
