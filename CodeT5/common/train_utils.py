...
def train_with_neg_sampling(args, model, train_examples: Examples, optimizer,
    ...
    for step, batch in tqdm(enumerate(train_dataloader), total=len(train_dataloader),mininterval=30):
        if skip_n_steps > 0:
            skip_n_steps -= 1
            continue
        ...
        if (step + 1) % args.gradient_accumulation_steps == 0:
            ...
            if args.local_rank in [-1, 0] and args.logging_steps > 0 and args.global_step % args.logging_steps == 0:
                tb_data = {
                    ...
                    'acc': tr_ac / min(step+1,args.logging_steps) / (
                            args.train_batch_size * args.gradient_accumulation_steps),
                    'loss': tr_loss / min(step+1,args.logging_steps)
                }
...
def train(args, train_examples, model, train_iter_method):
    ...
    :return:
    """
    model_output = os.path.join('./output', args.data_name)
    ...
    if args.local_rank in [-1, 0]:
        tb_writer = SummaryWriter(log_dir="./runs/{}_{}".format(args.data_name,exp_name))

    args.train_batch_size = args.per_gpu_train_batch_size * max(1, args.n_gpu)
    example_num = 2 * len(train_examples)  # 50/50 of pos/neg
    epoch_batch_num = example_num / args.train_batch_size  # 每一个epoch有多少batch
    t_total = epoch_batch_num // args.gradient_accumulation_steps * args.num_train_epochs  # 总共多少次参数更新
    ...
    step_bar = tqdm(initial=args.epochs_trained, total=t_total, desc="Steps",mininterval=30)
