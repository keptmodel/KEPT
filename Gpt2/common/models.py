...
from transformers import GPT2Tokenizer, GPT2Model, PreTrainedModel
...
class Gpt2(PreTrainedModel):
    def __init__(self, config, gpt2):
        ...
        self.ctokneizer = GPT2Tokenizer.from_pretrained(cgpt2_model,local_files_only = True)
        self.ctokneizer.pad_token = self.ctokneizer.eos_token
        self.ntokenizer = self.ctokneizer
        
        self.cgpt2 = GPT2Model.from_pretrained(cgpt2_model,local_files_only = True)
        self.ngpt2 = self.cgpt2
    ...
    def forward(
            ...
            relation_label=None):
        c_hidden = self.cgpt2(code_ids, attention_mask=code_attention_mask).last_hidden_state
        n_hidden = self.ngpt2(text_ids, attention_mask=text_attention_mask).last_hidden_state
    ...
    def create_nl_embd(self, input_ids, pos, attention_mask):
        n_hidden = self.ngpt2(input_ids, attention_mask=attention_mask).last_hidden_state
        pool_text_hidden = self.text_pooler(n_hidden)
    ...
    def create_pl_embd(self, input_ids, pos, attention_mask):
        c_hidden = self.cgpt2(input_ids, attention_mask=attention_mask).last_hidden_state
        pool_code_hidden = self.code_pooler(c_hidden)
