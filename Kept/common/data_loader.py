...
def read_OSS_examples(data_dir, type):
    commit_file = os.path.join(data_dir, "file_diffs.csv")
    issue_file = os.path.join(data_dir, "bug_reports.csv")
...
def load_examples(data_dir, model, num_limit,args, type='train'):
    ...
    code_kg = CodeKnowledgeGraph(args.code_kg_location, model.get_pl_tokenizer(),args)
    text_kg = TextKnowledgeGraph(args.text_kg_location, model.get_nl_tokenizer(),args)
    examples = Examples(raw_examples, code_kg, text_kg,args)
