...
from common.code_knowledge_graph import CodeKnowledgeGraph
from common.text_knowledge_graph import TextKnowledgeGraph
F_ID = 'id'
F_POS = 'pos'
F_TOKEN = 'tokens'
F_LOC = 'location'
...
class Examples:
    ...
    def __init__(self, raw_examples: List, code_kg:CodeKnowledgeGraph, text_kg:TextKnowledgeGraph,args):
        self.args = args
        self.code_kg = code_kg
        self.text_kg = text_kg
        self.ori_code_kg = CodeKnowledgeGraph("",code_kg.tokenizer,code_kg.args)
        self.ori_text_kg = TextKnowledgeGraph("",text_kg.tokenizer,text_kg.args)
        self.__size=0
        self.NL_index, self.PL_index, self.rel_index, self.rel_file_index,self.rev_rel_index = self.__index_exmaple(raw_examples)
    ...
    def __index_exmaple(self, raw_examples):
        ...
        for r_exp in raw_examples:
            ...
            if (pl_fid, pl_tks) in reverse_PL_index:
                ...
                pl_id = pl_id_max
                pl_id_max += 1
            PL_index[pl_id] = {F_TOKEN: pl_tks, F_ID: pl_id, F_LOC: pl_loc}
            ...
            rev_rel_index[pl_id].add(nl_id)
            rel_file_index[map_iss[nl_id]].add(map_file[pl_id])
        return NL_index, PL_index, rel_index, rel_file_index,rev_rel_index

    # depreted
    def _gen_feature(self, example, tokenizer):
        feature = tokenizer.encode_plus(example[F_TOKEN], max_length=self.args.length_limit, truncation=True,
    ...
    def _gen_seq_pair_feature(self, nl_id, pl_id, tokenizer):
        ...
        feature = tokenizer.encode_plus(
            ...
            max_length=self.args.length_limit,
    ...
    def __find_diff(self,tok,code_kg):
        for i in range(0,len(tok[F_INPUT_ID])):
            if tok[F_INPUT_ID][i]!=code_kg[F_INPUT_ID][i]:
                a = tok[F_INPUT_ID][i]
                b = code_kg[F_INPUT_ID][i]
                print("here problem")
                break

        a = tok[F_ATTEN_MASK]
        b = code_kg[F_ATTEN_MASK]
        #for i in range(0,len(tok[F_ATTEN_MASK])):
        #    for j in range(0,len(tok[F_ATTEN_MASK])):
        #        if a[j]==1:
        #            if b[i,j] != a[i] or b[j,i]!=a[i]:
        #                print("attention problem")
        #        if a[j]==0:
        #           if b[i,j]!=0 or b[j,i]!=0:
        #                print("attention problem2")
        if b.shape[0]!=len(a) or b.shape[1]!=len(a):
            print("attention problem")
                    
            
    def __update_feature_for_index(self, index, tokenizer, kg):
        for v in tqdm(index.values(), desc="update feature"):
            fres = self._gen_feature(v, tokenizer)
            test = kg.add_relation_encode(v[F_TOKEN],max_length = self.args.length_limit)
            id = fres[F_ID]
            index[id][F_INPUT_ID] = fres[F_INPUT_ID]
            index[id][F_ATTEN_MASK] = fres[F_ATTEN_MASK]
            index[id][F_POS] = fres[F_POS]
            test = kg.add_relation_encode(v[F_TOKEN],max_length = self.args.length_limit)
            index[id][F_ATTEN_MASK] = test[F_ATTEN_MASK]
            self.__find_diff(tok=fres,code_kg=test)
            
            
            
    def __update_feature_for_code(self, index,tok, n_thread=1):
        for v in tqdm(index.values(), desc="update code feature",maxinterval = 3600,mininterval=self.args.tqdm_interval):
            f = self.code_kg.add_relation_encode(v[F_TOKEN], pl_location=v[F_LOC], max_length=self.args.length_limit)
            id = v[F_ID]
    ...
    def __update_feature_for_text(self, index,tok, n_thread=1):
        for v in tqdm(index.values(), desc="update text feature",maxinterval = 3600,mininterval=self.args.tqdm_interval):
            f = self.text_kg.add_relation_encode(v[F_TOKEN], max_length=self.args.length_limit)
            id = v[F_ID]
            index[id][F_INPUT_ID] = f[F_INPUT_ID]
            index[id][F_POS] = f[F_POS]
            index[id][F_ATTEN_MASK] = f[F_ATTEN_MASK]
    ...
    def __update_embd_for_index(self, index, func,model):
        for id in tqdm(index, desc="update embedding",maxinterval = 3600,mininterval=self.args.tqdm_interval):
            feature = index[id]
    ...
    def __get_feature(self,id_list,index):
        input_ids, input_pos, att_masks = [], [], []
        ori_input_ids,ori_input_pos,ori_masks = [],[],[]
        for id in id_list:
            input_ids.append(torch.tensor(index[id][F_INPUT_ID]))
            if index[id][F_POS]is not None:
                input_pos.append(torch.tensor(index[id][F_POS]))
            if F_ATTEN_MASK in index[id]:
                att_masks.append(torch.tensor(index[id][F_ATTEN_MASK]))
            ori_input_ids.append(torch.tensor(index[id]["ori_"+F_INPUT_ID]))
            ori_input_pos.append(torch.tensor(index[id]["ori_"+F_POS]))
            ori_masks.append(torch.tensor(index[id]["ori_"+F_ATTEN_MASK]))
        input_ids_tensor = torch.stack(input_ids)
        input_pos_tensor = None
        if len(input_pos)>0:
            input_pos_tensor = torch.stack(input_pos)
        ...
        if len(att_masks)>0:
            att_tensor = torch.stack(att_masks)
        ori_att_tensor = torch.stack(ori_masks)
        ori_pos_tensor = torch.stack(ori_input_pos)
        ori_input_tensor = torch.stack(ori_input_ids)
        return (input_ids_tensor, input_pos_tensor, att_tensor),(ori_input_tensor,ori_pos_tensor,ori_att_tensor)
    ...
    def online_neg_sampling_dataloader(self, batch_size):
        ...
        return dataset
    def make_positive_batch(self, batch: Tuple,model):
        nl_ids = batch[0].tolist()
        pl_ids = batch[1].tolist()
        pl,pl_ori = self.__get_feature(pl_ids,self.PL_index)
        nl,nl_ori = self.__get_feature(nl_ids,self.NL_index)
        
        pl_ids = pl[0].to(model.device)
        pl_pos = pl[1].to(model.device)
        pl_attn = pl[2].to(model.device)
        
        nl_ids = nl[0].to(model.device)
        nl_pos = nl[1].to(model.device)
        nl_attn = nl[2].to(model.device)
        
        extended_batch = batch[2].to(model.device)
        return {
            "text_ids": nl_ids,
            "text_pos": nl_pos,
            "text_attention_mask": nl_attn,
            "code_ids": pl_ids,
            "code_pos": pl_pos,
            "code_attention_mask": pl_attn,
            "relation_label": extended_batch
        }
    def make_make_sampling_batch(self, batch: Tuple,model):
        nl_ids = batch[0].tolist()
        pl_ids = batch[1].tolist()
        
        pl,pl_ori = self.__get_feature(pl_ids,self.PL_index)
        nl,nl_ori = self.__get_feature(nl_ids,self.NL_index)
        pl_ids = torch.cat((pl[0],pl_ori[0]),dim=0).to(model.device)
        pl_pos = torch.cat((pl[1],pl_ori[1]),dim=0).to(model.device)
        pl_attn = torch.cat((pl[2],pl_ori[2]),dim=0).to(model.device)
        nl_ids = torch.cat((nl[0],nl_ori[0]),dim=0).to(model.device)
        nl_pos = torch.cat((nl[1],nl_ori[1]),dim=0).to(model.device)
        nl_attn = torch.cat((nl[2],nl_ori[2]),dim=0).to(model.device)

        zeros = torch.zeros_like(batch[2])
        extended_batch = torch.cat((batch[2], zeros), dim=0).to(model.device)
        return {
            "text_ids": nl_ids,
            "text_pos": nl_pos,
            "text_attention_mask": nl_attn,
