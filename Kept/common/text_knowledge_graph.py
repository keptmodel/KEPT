...
class TextKnowledgeGraph:
    ...
    def add_relation_encode(self, text: str, max_length=256):
        ...
        if len(tokens) >= max_length - 4:
            ...
            tokens = [self.tokenizer.cls_token,'<encoder-only>',self.tokenizer.sep_token] + tokens + [self.tokenizer.sep_token]
        ...
        for token in tag_tokens:
            if token == self.entity_flag:
                if last_index is None:
                    ...
                    entity_loc_list.append(((last_index + 3, len(tokens) - 1 + 3), entity_list[entity_count]))
                    entity_count += 1
                    last_index = None
            else:
                tokens.append(token)
