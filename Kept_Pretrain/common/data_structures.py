...
from pretrain.common.utils import format_batch_input
...
class Examples:
    ...
    def _gen_feature(self, example, tokenizer):
        text = example[F_TOKEN]
        text_tokens = tokenizer.tokenize(text)[:512-4]
        text_tokens =[tokenizer.cls_token, '<encoder-only>', tokenizer.sep_token]+text_tokens+[tokenizer.sep_token]
        text_ids = tokenizer.convert_tokens_to_ids(text_tokens)
        padding_length = 512 - len(text_ids)
        att = [1 for i in range(0,len(text_ids))]
    ...
    def update_features(self, model, n_thread=1):
        ...
        :return:
        """
        print(len(self.PL_index))
        print(len(self.NL_index))
    ...
    def __update_embd_for_index(self, index, sub_model):
        for id in tqdm(index, desc="update embedding"):
            ...
            embd = sub_model(input_tensor,attention_mask=mask_tensor)[0][:,0,:]
    ...
    def _id_to_feature(self, id_tensor: Tensor, index):
        ...
        for id in id_tensor.tolist():
            ...
            if F_ATTEN_MASK in index[id] and index[id][F_ATTEN_MASK] is not None:
    ...
    def random_neg_sampling_dataloader(self, batch_size):
        print("start random_neg_sampling_dataloader,size:{0}".format(self.__size))
        pos, neg = [], []
    ...
    def __rank_and_select(self, model, batch_size, top_n, search_space_percent=1.0):
        """
