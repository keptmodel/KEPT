...
//		System.out.println(concernedCommits.toString());
	...
	public static void processBugReports() {
		...
		List<String> bugCommitLink = new ArrayList<String>();
		for (Bug bug : bugs) {
...
//			System.out.println(bug.id + "\t" + clts.toString());
			...
			for(String commit:bug.commits){
				line_commit+="\t"+commit;
			}
			bugCommitLink.add(line_commit);
		...
		filename = main.Main.settings.get("workingLoc") + File.separator + "bugCommitIndex.txt";
		WriteLinesToFile.writeLinesToFile(bugCommitLink, filename);
	}
	public static void processCommitLink(){
		List<String> bugCommitHunkLink = new ArrayList<String>();
		List<String> bugCommitFileLink = new ArrayList<String>();
		String filename = main.Main.settings.get("workingLoc") + File.separator + "bugCommitIndex.txt";
		...
		while (index < lines.size()) {
			...
			String hunk_line=""+bid;
			String file_line=""+bid;
			HashSet<String> fileSet = new HashSet<>();
			for (int i = 1; i < splits.length; i++){
				String hash = splits[i];
				...
				for(Hunk hunk:hunks){
					...
					String hunkid = hash + "@" + hunk.preChangeSet + "@" + hunk.postChangeSet + "@" + sourceFile + "@" + hunk.bs + "@" + hunk.bl + "@" + hunk.as + "@" + hunk.al + ".txt";
					hunk_line+="\t"+hunkid;

					String fileid = hash + "@"+sourceFile;
					if(!fileSet.contains(fileid)){
						file_line+="\t"+fileid;
					}
				}
			}
			bugCommitHunkLink.add(hunk_line);
			bugCommitFileLink.add(file_line);
			index++;
		}
		filename = main.Main.settings.get("workingLoc") + File.separator + "bugCommitHunkIndex.txt";
		WriteLinesToFile.writeLinesToFile(bugCommitHunkLink, filename);
		filename = main.Main.settings.get("workingLoc") + File.separator + "bugCommitFileIndex.txt";
		WriteLinesToFile.writeLinesToFile(bugCommitFileLink, filename);
	}
