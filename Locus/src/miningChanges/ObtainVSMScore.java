...
public class ObtainVSMScore {
	...
	public HashMap<Integer, String> hunkChangeFileMap;
	public HashMap<Integer, String> hunkChangeHunkMap;
...
//			System.out.println(line);
	...
	public void loadHunkFiles() {
		...
		hunkChangeFileMap = new HashMap<>();
		hunkChangeHunkMap = new HashMap<>();

		for (String line : lines) {
			String[] split = line.split("\t");
			hunkIndex.add(split[0]);
			hunkChangeMap.put(index, split[0].split("@")[0]);
			hunkChangeFileMap.put(index,split[0].split("@")[0]+"@"+split[0].split("@")[3]);
			hunkChangeHunkMap.put(index,split[0]);
...
//			System.out.println(bugCLTFreq.toString());
	...
	public HashMap<String,Double> getRankingResults(HashMap<Integer,Double> results, boolean isChangeLevel,ChangeLevel level) {
		HashMap<String,Double> sourceSimis = new HashMap<String,Double>();
		for (int hunk : results.keySet()) {
			String eid = "";
			if (isChangeLevel){
				if(level==ChangeLevel.commit){
					eid = hunkChangeMap.get(hunk);
				}
				else if (level==ChangeLevel.file){
					eid = hunkChangeFileMap.get(hunk);
				}
				else{
					eid = hunkChangeHunkMap.get(hunk);
				}
...
//			if (!semanticHids.contains(hunk)) continue;
	...
	public HashMap<Integer, HashMap<String, Double>> getResults(boolean isChangeLevel, ChangeLevel level) {
		...
		String resultNLFile="";
		String resultCLTFile="";
		String resultFile = "";
		if(isChangeLevel){
			if(level==ChangeLevel.commit){
				resultNLFile = loc + File.separator + "resultsNL_" + "changecommit" + ".txt";
				resultCLTFile = loc + File.separator + "resultsCLT_" + "changecommit" + ".txt";
				resultFile = loc + File.separator + "results_" + "changecommit" + ".txt";
			}
			if(level==ChangeLevel.file){
				resultNLFile = loc + File.separator + "resultsNL_" + "changefile" + ".txt";
				resultCLTFile = loc + File.separator + "resultsCLT_" + "changefile" + ".txt";
				resultFile = loc + File.separator + "results_" + "changefile" + ".txt";
			}
			if(level==ChangeLevel.hunk){
				resultNLFile = loc + File.separator + "resultsNL_" + "changehunk" + ".txt";
				resultCLTFile = loc + File.separator + "resultsCLT_" + "changehunk" + ".txt";
				resultFile = loc + File.separator + "results_" + "changehunk" + ".txt";
			}
		}
		else{
			resultNLFile = loc + File.separator + "resultsNL_" + "file" + ".txt";
			resultCLTFile = loc + File.separator + "resultsCLT_" + "file" + ".txt";
			resultFile = loc + File.separator + "results_" + "file" + ".txt";
		}
		...
		for (int b = 0; b < bugRank.size(); b++) {
			...
			HashMap<String,Double> entitySimisNL = getRankingResults(resultNL, isChangeLevel,level);
			HashMap<String,Double> entitySimisCLT = getRankingResults(resultCLT, isChangeLevel,level);			
	...
	public HashMap<Integer, HashMap<String, Double>> obtainSimilarity(boolean isChangeLevel) {
		...
		return getResults(isChangeLevel,ChangeLevel.commit);
	}
	public HashMap<Integer, HashMap<String, Double>> obtainSimilarity(boolean isChangeLevel, ChangeLevel level) {
		loadBugFiles();
		System.out.println("loaded bugFiles.");
		loadCLTIndex();
		System.out.println("loaded CLTIndex.");
		loadHunkFiles();
		System.out.println("loaded hunkFiles.");
		return getResults(isChangeLevel,level);
	}
