...
public class ProduceChangeHunkLevelResults {
	...
	public boolean loadOracles() {
		String filename = loc + File.separator + "bugCommitHunkIndex.txt";
...
//			System.out.println(lines.get(index));
	...
	public void loadResults() {
		...
		String resultFile = loc + File.separator + "results_changehunk" + ".txt";
		File file = new File(resultFile);
		if (!file.exists()) {
			ObtainVSMScore ovs = new ObtainVSMScore();
			hunkResults = ovs.obtainSimilarity(true,ObtainVSMScore.ChangeLevel.hunk);
	...
	public void loadRevisionTime() throws ParseException {
		List<String> lines = FileToLines.fileToLines(main.Main.settings.get("repoDir") + File.separator + "logOneline.txt");
	...
	public void integrateResults() {
		...
		for (Bug bug : bugs) {
			...
			for (String change : results.keySet()) {
				if (revisionTime.containsKey(change.split("@")[0]) && revisionTime.get(change.split("@")[0]) < bug.modifyTime)
					changeRanks.add(new Pair<String,Long>(change, revisionTime.get(change.split("@")[0])));
...
//				System.out.println(change + "\t" + revisionTime.get(change));
			...
			for (int i = 0; i < finalRanks.size(); i++) {
				...
				if (inducingRevisions.get(bid).contains(finalRanks.get(index).getKey())){
					rank.add(i);
					inducingRevisions.get(bid).remove(finalRanks.get(index).getKey());
				}
			}
			for (String hunk : inducingRevisions.get(bid)) {
				rank.add(Integer.MAX_VALUE);
			}
			ranks.add(rank);
		}
		...
		System.out.println();
		String filename = main.Main.settings.get("workingLoc") + File.separator + "changeHunkLevelResults.txt";
