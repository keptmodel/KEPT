...
def metrics(data_dpath, ranks_file, br_to_skip, br2type, use_types):
    ...
    mrr = sum(rrs) / len(rrs) if len(rrs)>0 else 1
    map = sum(aps) / len(aps) if len(aps)>0 else 1
    p1_avg = sum(p1) / len(p1) if len(p1)>0 else 1
    p3_avg = sum(p3) / len(p3) if len(p3)>0 else 1
    p5_avg = sum(p5) / len(p5) if len(p5)>0 else 1
    count = len(rrs)
...
def parse_args():
    ...
    parser.add_argument('--dpath', default="../data/")
    return parser.parse_args()


if __name__ == '__main__':
    args = parse_args()
    results_dpath = args.dpath
    evaluate_on = 'test'
    ...
    try:
        for eval_type in types:
            #for project_name in projects:
                ...
                br2type = load_br2type(project_dpath)
